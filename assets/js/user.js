...
$(function(){
	//タイトルを追加する
		$('.user-article').on('click','.add-title',function(){
			...
			$('.subtitles').animate({height:'135'},200);
		});
	//削除時のタイトルNo.の振り直し
	...
	//画像／動画を追加する
		$('.user-article').on('click','.add-load',function(){
			...
			$('.user-article ul > li:last').before('<li class="subloads"><fieldset class="uploads"><legend>サブ画像／動画'+subLoadNum+'</legend><div><label class="sub-image-label" for="article_sub_image_'+ subLoadArray[subLoadNum] + '">画像はこちらから</label><br><input class="sub-image-input mb30" name="article[sub_image_' + subLoadArray[subLoadNum] + ']" id="article_sub_image_'+ subLoadArray[subLoadNum] + '" type="file" value="画像アップロード"></div><div><label class="sub-movie-label" for="article_movie_thumbnail_0' + subLoadNum + '">動画はこちらから</label><br><textarea name="article[movie_thumbnail_0'+subLoadNum+']" id="article_movie_thumbnail_0' + subLoadNum + '" rows="5" cols="52"></textarea></div></fieldset><button class="delete-load-button" type="button">削除する</button></li>');
			$('.subloads').animate({height:'280'},200);
		});
	//削除時のタイトルNo.の振り直し
	...
	//テキストを追加する
		$('.user-article').on('click','.add-text',function(){
			...
			$('.subtexts').animate({height:'385'},200);
		});
	//削除時のタイトルNo.の振り直し
